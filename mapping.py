...
pressure_range = np.arange(0, 5.1, 0.25)
...
plt.plot(pressure_range, ideal_stiffness)
# plt.plot(pressure_range, noisy_stiffness, label='Noisy Mapping')
plt.title("Experimentally determined mapping", size=18)
plt.xlabel('Knuckle Pressure (psi)', size=16)
plt.ylabel('Joint Stiffness (Nmm/rad)', size=16)
plt.xlim(0, 5)
# plt.legend()
plt.show()
